...
public class CalendarActivity extends BaseActivity {
    ...
    private final Set<String>        dotDates   = new HashSet<>(); // dates that have ≥1 event

    private DayAdapter   dayAdapter;
    private EventAdapter eventAdapter;

    /** All events for the currently displayed month — used for calendar sync. */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (btnSync != null) btnSync.setOnClickListener(v -> onSyncClicked());
    ...
    private void loadMonthDots() {
        ...
        db.collection("events")
                ...
                .addOnFailureListener(e ->
                        Toast.makeText(this, "שגיאה בטעינת יומן: " + e.getMessage(), Toast.LENGTH_LONG).show());
    }
    ...
    private void showAddEventDialog() {
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("שמור", (d, w) -> {
                    ...
                    if (title.isEmpty()) {
                        Toast.makeText(this, "נא להזין שם אירוע", Toast.LENGTH_SHORT).show();
                        return;
                    }
                    String notes = etNotes.getText() != null
                            ? etNotes.getText().toString().trim() : "";
                    saveEvent(title, notes, target, chosenTime[0]);
                })
    ...
    private void saveEvent(String title, String notes, String date, String time) {
        ...
        db.collection("events").add(data)
                .addOnSuccessListener(ref -> {
                    dotDates.add(date);
                    buildGrid();
                    if (date.equals(selectedDate)) loadDayEvents(date);
                })
                .addOnFailureListener(e ->
        ...
        public void onBindViewHolder(@NonNull VH h, int position) {
            ...
            h.itemView.setOnClickListener(v -> {
                ...
                selectedDate = dateStr(
                        ...
                        item.day);
                tvEventsHeader.setText("אירועים — " + humanDate(selectedDate));
    ...
    private void confirmAndSync() {
        ...
        new AlertDialog.Builder(this)
                ...
                .setPositiveButton("סנכרן", (d, w) -> exportEventsToDeviceCalendar())
                .setNegativeButton("ביטול", null)
                .show();
    }

    private void exportEventsToDeviceCalendar() {
        // Find primary device calendar
    ...
    private class EventAdapter extends RecyclerView.Adapter<EventAdapter.VH> {
        ...
        public void onBindViewHolder(@NonNull VH h, int position) {
            ...
            // Long-press → delete (own entry or parent)
            boolean canDelete = currentUid.equals(e.getAuthorUid()) || "הורה".equals(userRole);
            h.itemView.setOnLongClickListener(canDelete ? v -> {
                new AlertDialog.Builder(CalendarActivity.this)
                        .setTitle("מחיקת אירוע")
                        .setMessage("למחוק את \"" + e.getTitle() + "\"?")
                        .setPositiveButton("מחק", (d, w) -> deleteEvent(e.getEventId(), e.getDate()))
                        .setNegativeButton("ביטול", null)
                        .show();
                return true;
            } : null);
